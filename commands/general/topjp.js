...
module.exports = {
    ...
    async exec({ client, msg, args, isOwner }) {
        ...
        let url = `https://bestdori.com/api/eventtop/data?server=0&event=254&mid=0&interval=0`;
        let isLow = false;
        let isErr = false;
        if(args[0]!=undefined&&args[0]!=null&&args[0]!=''){
            if(parseInt(args[0])>10){
                if(
                    ...
                    {
                isLow = true;
                url = 'https://bestdori.com/api/tracker/data?server=0&event=254&tier='+args[0];
                }
                else{
                    isLow = true;
                    isErr = true;
...
`
    ...
    await client.sendMessage(msg.from, { text: `Gunakan .top {number} , 
number =
10,20,30,40,50,100,200,300,400,500,
1000,2000,5000,10000,20000,30000,50000,70000,
100000,300000
